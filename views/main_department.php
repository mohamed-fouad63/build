...
<body>
                ...
                <li>
                    <div class="btn-group bt_div">
                        <button class="btn" tabindex="0" aria-controls="example" data-bs-toggle="modal"
                            data-placement="right" title="اضافه اداره عامه جديده" id=""
                            data-bs-target="#Add_Main_Department_Modal">
                            <label class="btn btn-primary">اضافه اداره عامه جديده</label>
                        </button>
                    </div>
                </li>
            <?php } ?>
    ...
    <div class="pcoded-main-container">
        ...
        <?php include("../component/modals/building_group/add_main_department_modal.php") ?>
        <?php include("../component/modals/building_group/edit_main_department_modal.php") ?>
        <?php include("../component/modals/building_group/del_main_department_modal.php") ?>
    ...
    <script src="../js/building_group/add_main_department.js"></script>
    <script src="../js/building_group/edit_main_department.js"></script>
    <script src="../js/building_group/del_main_department.js"></script>
    <script>
        function get_sector_main_departments() {
            var container_sector_main_departmenst = "";
            $.post(
                "../api/building/get_sector_main_departments.php",
                {
                ...
                function (data) {
                    $.each(data, function (key, val) {
                        main_department_number = key + 1;
                        container_sector_main_departmenst += `
                <div class='col-md-12 col-xl-2 text-center'>
    <form class="card" method="post" target="_balnk" action="branch_department.php">
                        <div class="card-header">
                        <button type="button" class="btn btn-outline-info">${main_department_number}</button>
                        <?php

                        ?>
                        <button type="button" class="btn btn-outline-success" onclick='user_details_modal(${key})' title="تعديل اسم الاداره العامه">
                        <i class="bi bi-pencil-fill"></i>
                        </button>
                        <button type="button" class="btn btn-outline-danger" onclick='del_main_department_modal(${key})' title="حذف الاداره العامه">
                        <i class="bi bi-trash"></i>
                        </button>
                    </div> 
  <div class="card-body">
  <i class="bi bi-diagram-3-fill me-2 fs-1"></i>
    <h5 class="card-title fs-3">${val['main_department_name']}
    <input type="hidden" name="main_department_id"  data-main_department_id="${val['main_department_id']}" value="${val['main_department_id']}">
    <input type="hidden" name="main_department_name" data-main_department_name="${val['main_department_name']}" value="${val['main_department_name']}">
...
<input type="submit" class="btn btn-primary" value="التفاصيل">
    ...
    <script>
        function user_details_modal(e) {
            ...
            main_department_name = $("form:eq(" + e + ") input[data-main_department_name]").data("main_department_name");
            ...
            $('#edit_main_department_input').val(main_department_name);
            $('#edit_main_department_input').data('old_main_department_name', main_department_name);
            $('#edit_main_department_input').data('main_department_id', main_department_id);
            $('#edit_main_department_input').data('sector_id', sector_id);
            $('#edit_main_department_input').data('floor_id', floor_id);
            $('#edit_main_department_input').data('building_id', building_id);
            $("#Edit_Main_Department_Modal").modal("show");
        }
    </script>
    <script>
        function del_main_department_modal(e) {
            e++;
            main_department_name = $("form:eq(" + e + ") input[data-main_department_name]").data("main_department_name");
            main_department_id = $("form:eq(" + e + ") input[data-main_department_id]").data("main_department_id");
            sector_id = $("form:eq(" + e + ") input[data-sector_id]").data("sector_id");
            ...
            $('#del_main_department_input').val(main_department_name);
            $('#del_main_department_input').data('main_department_name', main_department_name);
            $('#del_main_department_input').data('main_department_id', main_department_id);
            $('#del_main_department_input').data('sector_id', sector_id);
            $('#del_main_department_input').data('floor_id', floor_id);
            $('#del_main_department_input').data('building_id', building_id);
            $("#Del_Main_Department_Modal").modal("show");
        }
    </script>
</body>
